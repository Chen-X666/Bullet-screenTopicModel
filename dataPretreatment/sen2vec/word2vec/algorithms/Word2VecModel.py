...
from dataPretreatment.sen2vec.word2vec.util.DBUtil import DbPoolUtil
from dataPretreatment.sen2vec.word2vec.util.JiebaUtil import jieba_util
from dataPretreatment.sen2vec.word2vec.util.PropertiesUtil import prop
from gensim.models import word2vec
from dataPretreatment.sen2vec.word2vec.algorithms.OriginModel import origin_model
...
class Word2VecModel(object):
    def __init__(self):
        ...
        self.train_data_path = CWD+"/gen/train_data.txt"
        self.origin_model_path = CWD+"/model/word2vec_wx"
        self.model_path = CWD+"/model/word2vec_wx_bullet"
        self.model = None
        # 未登录词进入需考虑最小词频
        self.min_count = int(prop.get_config_value(CWD+"/config/w2v.properties", "min_count"))
    ...
    def get_train_data(self):
        ...
        print("创建额外语料训练数据")
        sql = "SELECT `text` " \
              "FROM bilibili_bullet.yiqing_bullet_table " \
              "WHERE bvno='BV117411671H' " \
              "AND sendUserCrc32id='10365390' " \
              "AND sendTime=1580721847"
        print(sql)
    ...
    def load_model(self):
        ...
        #self.model = gensim.models.KeyedVectors.load_word2vec_format(self.model_path,encoding='utf-8',limit=500000,unicode_errors='ignore')
        print("词嵌入模型加载完毕")
